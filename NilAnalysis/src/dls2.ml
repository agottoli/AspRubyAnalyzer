...
module LivenessAnalysis = struct
  ...
  let to_string t = strmap_to_string fact_to_s t
	...
	let asd lst =
				print_string "meeeeeet \n" ; 

		List.fold_left (fun acc map -> StrMap.fold update map acc)
		StrMap.empty lst
    ...
    | Assign(lhs, `ID_Var(`Var_Constant, rconst)) ->  print_string "constant\n";update_use lhs rconst map
		(*| Assign(lhs, MethodCall()) -> print_string targ; print_string " con methodcall\n";*) 
    ...
    | Yield(Some lhs, _ ) 
    | Assign(lhs, _ ) -> print_string "tutto il resto\n"; update_lhs Dead map lhs 
...
class safeLive ifs = object(s)		(* safeNil visitor *)
...
let main fname =
        ...
        let ifs, ofs = Liveness.fixpoint s in
        	...
        	print_hash ofs; 
        	print_string "--------------------------------------------\n";  
        let sn = ( new safeLive ifs :> cfg_visitor ) in
