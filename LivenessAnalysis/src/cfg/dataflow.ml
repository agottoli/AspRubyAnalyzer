...
(* auxiliary function that calculate our backward fixpoint *)
...
let rec super_fixpoint stmt in_tbl out_tbl =
		...
		match stmt.snode with
			...
			| Seq(list) ->  
				...
				(* iterate over it to calculate the facts of all the stmt of the list *)
  				List.iter (fun x -> 
  				    ...
  				    (* because it contains the information calculated up to now and it will be updated with the new facts at every analised stmt *)
  					Hashtbl.replace in_tbl x !ifacts; 
  					(* call this method recursively on the current stmt of the list to calculate its newfacts ( = out-facts table) *)
  					newfacts := super_fixpoint x in_tbl out_tbl;
			...
			| If(_, t, f) -> 
				(* the guard is ignored because analysed in the transfert when we calculate the out-facts table for the stmt *)
				
				(* we add the t and f branches with what we know before them (before the if) to in_tbl *)
				Hashtbl.replace in_tbl t !ifacts;
				Hashtbl.replace in_tbl f !ifacts;
				(* we call the super_fixpoint on both the branch to analyse them and calculate their out-facts *)
				let t_facts = super_fixpoint t in_tbl out_tbl in
				let f_facts = super_fixpoint f in_tbl out_tbl in
				...
				(* we have computed t_facts and f_facts independently based on what we know before the if stmt, now we have *)
				(* two StrMap and we have to join them into a single one following the if semantics *)
				(* and to calculate the out-facts table for the whole if stmt we call the transfer *)
  				DFP.transfer (DFP.join (t_facts :: (f_facts ::[]))) stmt (* returns a StrMap *)
			
			(* check if the stmt is a loop *)
			| For (_, _, b)		
			| While(_, b) ->
				(* we analyse only the body because the guard is analysed in the transfer when we calculate the out-facts table for the stmt *)
				...
				do_while (fun () -> 
		    		...
		    		b_facts := super_fixpoint b in_tbl out_tbl;
		    		(* calculate the newfacts (out-facts) of the loop at this iteration *)
					b_facts := DFP.join (!ifacts :: (!b_facts ::[]));
						...
						let finalfacts = 
							...
							List.fold_left ( fun acc x ->
												...
												(* it's the same for everyone *)
  												...
  												(* and calculate the out-facts table of the when stmt *)
  												let newfacts = super_fixpoint x in_tbl out_tbl in
  												(* replace the new out-facts for che when in out_table *)
  												Hashtbl.replace out_tbl x newfacts;
  												(* acculate all the out-facts of all the when stmt for calculate (after this fold_left) the out-facts of the case stmt *)
  												newfacts :: acc
											) [] st
						in
							(* we apply the join operator between all the elements of finalfacts *)
							(* so we calculate the out-facts of case, except for the guard... *)
							let tmp_fact = List.fold_left ( fun acc x ->
																DFP.join (acc :: (x :: []))
														) (List.hd finalfacts) (List.tl finalfacts) 
						in
							(* call the transfer so we calculate the guard out-facts *)
    ...
    (* calculate the fixpoint, it uses our auxiliary function super_fixpoint *)
    (* NOTE: in_tbl is the table with facts we know before calculate stmt *)
    (*       out_tbl is the table wit facts we know after calculate stmt *)
	...
	and fixpoint stmt =
		...
		let out_tbl = Hashtbl.create 127 in
			...
			Hashtbl.replace in_tbl stmt DFP.empty;
			(* calculate the new facts introduced by this stmt in output *)
			(* newfacts is what we know after stmt, the entire program! *)
			let newfacts = super_fixpoint stmt in_tbl out_tbl in
